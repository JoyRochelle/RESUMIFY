...
@section('title', 'New Template - Admin Dashboard')
    ...
    <form action="{{ route('admin.templates.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="admin-card-pad space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <x-user.form-input label="Name" name="name" :value="old('name', '')" required id="template-name" autocomplete="off" />
                <x-user.form-input label="Blade Path" name="blade_path" :value="old('blade_path', '')" placeholder="e.g. templates.minimal" required id="template-blade-path" autocomplete="off" />
            </div>
            ...
            <div>
                ...
                <textarea id="template-description" name="description" rows="3"
                          ...
                          class="w-full bg-surface border border-primary/10 rounded-lg px-4 py-2.5 text-sm font-label text-primary placeholder:text-primary/40 focus:outline-none focus:border-primary/30 resize-none">{{ old('description') }}</textarea>
        ...
        <div class="admin-card-pad space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <x-user.form-input label="Badge Label" name="badge" :value="old('badge', '')" maxlength="30" placeholder="e.g. New, Popular" id="template-badge" />
                <div>
        ...
        <div class="admin-card-pad space-y-4">
            ...
            <div x-data="{ preview: null }">
                ...
                <div x-show="preview" class="mt-3">
                    <img :src="preview" alt="Preview" class="h-32 w-auto rounded-lg object-cover border border-primary/10">
                </div>
            </div>
        </div>
        ...
        <div class="admin-card-pad space-y-4">
            ...
            <textarea id="template-style-config" name="style_config" rows="5"
                      ...
                      class="w-full bg-surface border border-primary/10 rounded-lg px-4 py-2.5 text-sm font-mono text-primary placeholder:text-primary/40 focus:outline-none focus:border-primary/30 resize-none">{{ old('style_config') }}</textarea>
            @error('style_config')
        ...
        <div class="flex items-center justify-between">
            ...
            <x-ui.loading-button loading-text="Creating..." icon="add">Create Template</x-ui.loading-button>
        </div>
    </form>
