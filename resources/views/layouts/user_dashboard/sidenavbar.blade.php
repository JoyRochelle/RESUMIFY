<aside class="hidden md:flex flex-col h-screen w-64 border-r border-primary/10 bg-surface-container-low p-6 space-y-8 sticky top-0 shrink-0">
    ...
    <nav class="flex-1 flex flex-col space-y-2">
        ...
        <a href="{{ route('manuscript') }}" class="flex items-center space-x-3 p-3 transition-all duration-300 {{ request()->routeIs('manuscript') ? 'text-primary font-bold bg-tertiary rounded-lg shadow-sm' : 'text-primary/60 hover:text-primary hover:translate-x-1' }}">
        ...
        <a href="{{ route('ai-assistant') }}" class="flex items-center space-x-3 p-3 transition-all duration-300 {{ request()->routeIs('ai-assistant') ? 'text-primary font-bold bg-tertiary rounded-lg shadow-sm' : 'text-primary/60 hover:text-primary hover:translate-x-1' }}">
            <span class="material-symbols-outlined" style="{{ request()->routeIs('ai-assistant') ? 'font-variation-settings: \'FILL\' 1;' : '' }}" data-icon="auto_fix_high">auto_fix_high</span>
            <span class="font-label tracking-wide">AI Assistant</span>
        </a>
        <a href="{{ route('settings') }}" class="flex items-center space-x-3 p-3 transition-all duration-300 {{ request()->routeIs('settings') ? 'text-primary font-bold bg-tertiary rounded-lg shadow-sm' : 'text-primary/60 hover:text-primary hover:translate-x-1' }}">
            <span class="material-symbols-outlined" style="{{ request()->routeIs('settings') ? 'font-variation-settings: \'FILL\' 1;' : '' }}" data-icon="settings">settings</span>
            <span class="font-label tracking-wide">Settings</span>
        </a>
        <a href="{{ route('help') }}" class="flex items-center space-x-3 p-3 transition-all duration-300 {{ request()->routeIs('help') ? 'text-primary font-bold bg-tertiary rounded-lg shadow-sm' : 'text-primary/60 hover:text-primary hover:translate-x-1' }}">
            <span class="material-symbols-outlined" style="{{ request()->routeIs('help') ? 'font-variation-settings: \'FILL\' 1;' : '' }}" data-icon="help_outline">help_outline</span>
    ...
    <div class="pt-6 border-t border-primary/10">
        ...
        <button onclick="event.preventDefault(); document.getElementById('logout-form').submit();" class="flex items-center space-x-3 p-3 mt-4 text-primary/60 hover:text-red-500 transition-colors w-full">
