<aside class="hidden md:flex flex-col h-screen w-64 border-r border-primary/10 bg-surface-container-low p-6 space-y-8 sticky top-0 shrink-0">
    ...
    <nav class="flex-1 flex flex-col space-y-1 relative -mx-6">
        ...
        <a href="{{ route('admin.monitor') }}" class="flex items-center space-x-3 px-6 py-3 transition-all duration-300 relative group {{ request()->routeIs('admin.monitor') ? 'text-primary font-bold bg-primary/5' : 'text-primary/60 hover:text-primary hover:bg-primary/5' }}">
            <span class="material-symbols-outlined {{ request()->routeIs('admin.monitor') ? 'icon-filled' : '' }} text-[20px]">monitor_heart</span>
            <span class="font-label text-sm tracking-wide">System Monitor</span>
            @if(request()->routeIs('admin.monitor'))
                <div class="absolute right-0 top-0 bottom-0 w-1 bg-secondary"></div>
            @endif
        </a>

        <a href="{{ route('admin.reports') }}" class="flex items-center space-x-3 px-6 py-3 transition-all duration-300 relative group {{ request()->routeIs('admin.reports*') ? 'text-primary font-bold bg-primary/5' : 'text-primary/60 hover:text-primary hover:bg-primary/5' }}">
            <span class="material-symbols-outlined {{ request()->routeIs('admin.reports*') ? 'icon-filled' : '' }} text-[20px]">bar_chart_4_bars</span>
            <span class="font-label text-sm tracking-wide">Revenue Report</span>
    ...
    <div class="pt-6 mt-auto space-y-3">
        <a href="{{ route('admin.reports') }}" class="w-full flex items-center justify-center space-x-2 py-3 px-4 rounded-lg bg-[#3f3129] text-[#fdf8f4] hover:bg-opacity-90 transition-all duration-300 font-label text-sm shadow-sm">
            <span class="material-symbols-outlined text-[18px]">bar_chart_4_bars</span>
