...
<aside class="fixed left-0 top-0 z-40 hidden h-screen w-64 shrink-0 flex-col space-y-8 overflow-y-auto border-r border-primary/10 bg-nav-footer p-6 md:flex">
    ...
    {{-- min-h-0 lets this column shrink below its content height, so a short
         viewport scrolls the nav list instead of pushing the Log Out button
         off-screen where it cannot be reached. --}}
    <nav class="flex-1 min-h-0 overflow-y-auto custom-scrollbar flex flex-col space-y-2" aria-label="Admin navigation">
    ...
    <div class="mt-auto shrink-0 border-t border-primary/10 pt-6">
