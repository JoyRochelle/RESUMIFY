...
@section('title', __('messages.resume.create.title') . ' - Resumify')

@section('content')
    <main class="flex-1 p-4 sm:p-6 md:p-12 max-w-5xl mx-auto w-full pb-24 md:pb-12">
        <x-user.page-header title="{{ __('messages.resume.create.title') }}" :back-url="route('user.manuscript')" />
            ...
            <form action="{{ route('resumes.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    <label for="title" class="mb-2 block text-xs font-bold uppercase tracking-wider text-primary/60">
                        {{ __('messages.resume.create.resume_title_label') }} <span class="text-red-500" aria-hidden="true">*</span>
                    </label>
                    <input type="text"
                           name="title"
                           id="title"
                           value="{{ old('title') }}"
                           placeholder="{{ __('messages.resume.create.title_placeholder') }}"
                           required
                           autocomplete="off"
                    ...
                    @else
                        <p id="title-hint" class="mt-2 text-sm text-primary/50">{{ __('messages.resume.create.title_hint') }}</p>
                    @enderror
                </div>

                <div>
                    <label for="template_id" class="mb-2 block text-xs font-bold uppercase tracking-wider text-primary/60">
                        {{ __('messages.resume.create.choose_template') }} <span class="text-red-500" aria-hidden="true">*</span>
                    </label>
                    <select name="template_id"
                        ...
                        <option value="">{{ __('messages.resume.create.select_template') }}</option>
                        @foreach($templates as $template)
                            <option value="{{ $template->id }}" {{ old('template_id') == $template->id ? 'selected' : '' }}>
                                {{ $template->name }}{{ $template->is_premium ? ' ' . __('messages.resume.create.premium_suffix') : '' }}
                            </option>
                        @endforeach
                    </select>
                    @error('template_id')
                    ...
                    @else
                        <p id="template-id-hint" class="mt-2 text-sm text-primary/50">{{ __('messages.resume.create.template_hint') }}</p>
                    @enderror
                </div>

                <div class="flex flex-col gap-3 border-t border-primary/10 pt-6 sm:flex-row sm:justify-end">
                    <a href="{{ route('user.manuscript') }}" class="inline-flex min-h-11 items-center justify-center rounded-lg border border-primary/15 px-5 py-2.5 text-sm font-bold text-primary transition hover:bg-primary/5 focus:outline-none focus:ring-2 focus:ring-secondary/40">{{ __('messages.resume.create.cancel') }}</a>
                    <x-ui.loading-button loading-text="{{ __('messages.resume.create.creating') }}" icon="add">{{ __('messages.resume.create.submit') }}</x-ui.loading-button>
                </div>
            </form>
        </section>
    </main>
