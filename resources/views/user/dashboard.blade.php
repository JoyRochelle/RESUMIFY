        ...
        <div id="create-modal-content"
            ...
            <div class="p-6 border-b border-primary/10 flex justify-between items-center bg-surface-container-low">
                ...
                <button onclick="closeCreateModal()"
                    class="text-primary/60 hover:text-primary transition-colors material-symbols-outlined rounded-full p-1 hover:bg-primary/5">close</button>
            </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                    @foreach ($templates as $template)
                        <form action="{{ route('resumes.store') }}" method="POST"
                            class="cursor-pointer group relative border border-primary/10 rounded-xl overflow-hidden hover:border-secondary transition-all hover:shadow-lg hover:-translate-y-1 bg-tertiary">
                            @csrf
                            <input type="hidden" name="title" value="My Professional Resume">
                            <input type="hidden" name="template_id" value="{{ $template->id }}">

                            <div
                                class="relative w-full aspect-[210/297] bg-surface-container-low overflow-hidden border-b border-primary/5">
                                ...
                                <div
                                    class="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-center pb-4 z-20">
                                    <span
                                        class="bg-secondary text-white text-xs px-3 py-1.5 rounded-full font-bold shadow-sm">Use
                                        Template</span>
                                </div>
                            </div>
                            <div class="p-4 flex justify-between items-center">
                                <div>
                                    <h4 class="font-bold text-sm text-primary group-hover:text-secondary transition-colors">
                                        {{ $template->name }}</h4>
                                    <p class="text-[11px] text-primary/60 mt-1 line-clamp-1">
                                        {{ $template->is_premium ? 'Premium' : 'Free' }}</p>
                                </div>
                            </div>

                            <button type="submit"
                                class="absolute inset-0 w-full h-full opacity-0 z-30 cursor-pointer"></button>
                        </form>
                    @endforeach
                </div>
            </div>
    ...
    <div id="delete-modal"
        ...
        <div id="delete-modal-content"
            ...
            <div class="p-6 bg-surface">
                <p class="text-primary/80 mb-6">Are you sure you want to delete this resume? This action cannot be undone.
                </p>
