...
@section('title', __('messages.tickets.show.ticket_number', ['id' => substr($ticket->id, -8)]) . ' - Resumify')

@section('content')
    <main class="flex-1 overflow-y-auto custom-scrollbar bg-primary/5 pb-20 md:pb-0">

        <div class="max-w-5xl mx-auto px-4 sm:px-6 md:px-12 py-8 md:py-16">

            {{-- Breadcrumb --}}
            <div class="flex items-center justify-between mb-8">
                <div>
                    <div class="flex items-center space-x-2 text-sm font-label text-primary/50 mb-2">
                        <a href="{{ route('user.help') }}" class="hover:text-primary transition-colors">{{ __('messages.tickets.show.breadcrumb_help') }}</a>
                        <span class="material-symbols-outlined text-[14px]">chevron_right</span>
                        <a href="{{ route('help.tickets') }}" class="hover:text-primary transition-colors">{{ __('messages.tickets.show.breadcrumb_my_tickets') }}</a>
                        <span class="material-symbols-outlined text-[14px]">chevron_right</span>
                        <span class="text-primary">{{ __('messages.tickets.show.ticket_number', ['id' => substr($ticket->id, -8)]) }}</span>
                    </div>
            ...
            <div class="pt-6 mt-6 bg-white rounded-3xl border border-primary/5 shadow-sm p-6 text-sm font-label text-primary/60 flex items-center justify-between">
                <div class="flex items-center space-x-2">
                    ...
                    <span>{{ __('messages.tickets.show.opened_on', ['date' => $ticket->created_at->format('d M Y')]) }}</span>
                </div>
            </div>
